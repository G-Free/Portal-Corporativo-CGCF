...
const BorderMap: React.FC = () => {
  ...
  const provinces = useMemo(() => {
    return ['Todas', ...Array.from(new Set(BORDER_POSTS.map(p => p.province))).sort()];
  }, []);
  ...
  const filteredPosts = useMemo(() => {
    return BORDER_POSTS.filter(post => {
      ...
      const matchesProvince = selectedProvince === 'Todas' || post.province === selectedProvince;
      ...
      <section className="bg-[#003366] pt-16 pb-24 border-b-4 border-[#C5A059]">
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex items-center gap-2 text-[10px] font-bold text-slate-300 uppercase tracking-widest mb-10">
             ...
             <span>/</span>
             <span className="text-[#C5A059]">Rede de Fronteiras</span>
          </div>
          ...
          <aside className="lg:col-span-3 space-y-8">
            <div className="bg-white p-8 rounded-sm border border-slate-200 shadow-sm">
              ...
              <div className="space-y-6">
                <div>
                  <label className="block text-[10px] font-black uppercase text-slate-400 mb-3 tracking-wider">Província</label>
                  <select 
                ...
                <div>
                  <label className="block text-[10px] font-black uppercase text-slate-400 mb-3 tracking-wider">Tipo de Posto</label>
            ...
            <div className="bg-[#003366] p-8 rounded-sm text-white">
               <h4 className="text-[#C5A059] text-[10px] font-black uppercase tracking-widest mb-4">Apoio Técnico</h4>
               <p className="text-xs font-medium leading-relaxed opacity-80 mb-6">
                 Dificuldades em localizar um posto? Contacte a nossa linha de apoio.
               </p>
               <Link to="/contacto" className="text-[10px] font-black uppercase border-b border-[#C5A059] pb-1 hover:text-[#C5A059] transition-colors">
                 Fale Connosco
               </Link>
            </div>
          </aside>
          ...
          <div className="lg:col-span-9 space-y-8">
            ...
            <div className="grid md:grid-cols-2 gap-6">
              {filteredPosts.map((post) => (
                <div key={post.id} className="bg-white border border-slate-200 rounded-sm overflow-hidden hover:border-[#C5A059] transition-all group flex flex-col">
                  <div className="p-8">
                    <div className="flex justify-between items-start mb-6">
                      <div className="p-3 bg-slate-50 border border-slate-100 rounded text-[#003366] group-hover:bg-[#003366] group-hover:text-white transition-all">
                         ...
                         {post.type === 'MARITIME' && <Anchor className="w-6 h-6" />}
                         {post.type === 'LAND' && <Navigation className="w-6 h-6" />}
                      </div>
                      <div className={`flex items-center gap-1.5 text-[9px] font-black uppercase tracking-widest ${post.status === 'OPEN' ? 'text-emerald-600' : 'text-amber-600'}`}>
                        {post.status === 'OPEN' ? <CheckCircle2 className="w-3 h-3" /> : <AlertCircle className="w-3 h-3" />}
                        {post.status === 'OPEN' ? t('status_open') : t('status_limited')}
                      </div>
                    </div>
                    <h3 className="text-[#003366] text-xl font-bold uppercase italic tracking-tight mb-2 group-hover:text-[#C5A059] transition-colors">{post.name}</h3>
                  ...
                  <div className="mt-auto bg-slate-50 p-6 flex flex-wrap gap-2 border-t border-slate-100">
                     {post.agencies.slice(0, 4).map(a => (
                       <span key={a} className="bg-white border border-slate-200 text-[#003366] text-[8px] font-black px-2 py-1 rounded-sm">{a}</span>
                     ))}
                     {post.agencies.length > 4 && <span className="text-[8px] font-black text-slate-400">+{post.agencies.length - 4}</span>}
                  </div>
                </div>
              ))}
            </div>
            ...
            {filteredPosts.length === 0 && (
              <div className="text-center py-24 bg-white rounded-sm border border-slate-100">
                 ...
                 <p className="text-slate-400 font-bold uppercase tracking-widest text-xs">Nenhum posto encontrado para esta pesquisa.</p>
              </div>
            )}
          </div>
        </div>
