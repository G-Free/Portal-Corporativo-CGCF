...
const Navbar: React.FC = () => {
  ...
  const navLinks = [
    { name: t("nav_home"), path: "/" },
    {
      ...
      path: "/sobre",
      dropdown: [
        { name: t("nav_about_who"), path: "/sobre", icon: Shield },
        { name: t("nav_about_comp"), path: "/composicao", icon: LayoutGrid },
        { name: t("nav_about_coord"), path: "/coordenadores", icon: Users },
        { name: t("nav_about_sec"), path: "/secretario", icon: Award },
        { name: t("nav_about_reports"), path: "/relatorios", icon: FileText },
      ],
    },
  ...
  return (
    <header className="fixed top-0 w-full z-[100]">
      ...
      >
        <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
          <Link to="/" className="flex items-center gap-2 md:gap-3">
            <div className="flex flex-col  border-slate-200 pl-3">
          ...
          <nav className="hidden lg:flex items-center gap-1">
            {navLinks.map((link) => (
              ...
              >
                <Link
                  to={link.path}
                  className={`px-3 py-2 text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-1 ${
                    isActive(link.path) ||
                    (link.dropdown &&
      ...
      {/* Mobile Menu */}
      <div
        className={`fixed inset-0 bg-white z-[110] transition-all duration-500 lg:hidden ${isOpen ? "translate-x-0" : "translate-x-full"}`}
      >
          ...
          <nav className="flex flex-col gap-4">
            {navLinks.map((link) => (
              <div key={link.name}>
                <Link
                  to={link.path}
                  className="text-xl font-black uppercase italic text-[#003366] py-2 block border-b border-slate-100"
                >
                  {link.name}
                </Link>
                {link.dropdown && (
                  <div className="pl-4 mt-2 flex flex-col gap-2">
                    {link.dropdown.map((sub) => (
                      <Link
                        key={sub.name}
                        to={sub.path}
                        className="text-xs font-bold uppercase text-slate-400 py-2"
                      >
                        {sub.name}
                      </Link>
                    ))}
